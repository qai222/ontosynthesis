...
def export_owlready2(onto: Ontology, data: dict[str, dict], existing_iri=None, existing_onto_classes_names=None):
...
def owx_to_owlready2(bundle_files=True):
    ...
    registered_iris = set()
    registered_onto_classes_names = set()
    bundle = []
    for fn in glob.glob(f"{dump_dir}/*.owx"):
        owx_data = parse_owx(fn)
        block_code, registered_iris, registered_onto_classes_names = export_owlready2(
            ontology, owx_data,
            existing_iri=registered_iris, existing_onto_classes_names=registered_onto_classes_names
        )
        bundle += block_code
